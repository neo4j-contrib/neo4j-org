        ...
        var diagram = function ( selection )
        {
            selection.each( function ( model )
            {
                ...
                function renderBoundVariables(className) {
                    ...
                    var allNodes = d3.values(model.nodeList());


                    view.selectAll("text." + className).remove();
                    //view.selectAll("svg:a").remove();                        
                    
                    var boundVariables = view.selectAll("text." + className).data(allNodes.filter(label));
